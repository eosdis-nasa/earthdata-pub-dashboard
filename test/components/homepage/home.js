...
test('EDPUB-799 Home page contains distribution error report', async (t) => {
  const dist = {
    apiGateway: {
  ...
  const earthdatapubInstance = {stackName: 'stackName'};
  ...
  const home = shallow(
    <Home
      ...
      granules={granules}
      stats={stats}
      earthdatapubInstance={earthdatapubInstance}
      location={{search: ''}}
    />
  );
