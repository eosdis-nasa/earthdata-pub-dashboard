...
test('reducers/earthdata-pub-instance/add_instance_meta', (t) => {
  const initialState = {};
  const action = {
...
test('handles earthdata-pub.stackName data', (t) => {
  const initialState = {};
  const action = {
    type: ADD_INSTANCE_META,
    data: {earthdatapub: {stackName: 'fakeStack'}}
  };
  const expectedState = {
    stackName: 'fakeStack'
  };
...
test('Updated multiple top level instance metadata on initial state', (t) => {
  ...
  const action = {
    ...
    data: {
      ...
      earthdatapub: {stackName: 'new stackName'}
    }
  };
