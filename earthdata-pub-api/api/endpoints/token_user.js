 ...
 * @param {Object} oAuth2User - an oAuth user object
 * @param {Object} response - an express response object
 * @returns {Promise<Object>} the promise of express response object
 */
async function token(event, oAuth2User, response) {
  ...
  if (code) {
    try {
      const {
        ...
        username,
        expirationTime
      } = await oAuth2User.getAccessToken(code);
...
/**
 * Handle refreshing tokens with OAuth user
 *
 * @param {Object} request - an API Gateway request
 * @param {OAuth2} oAuth2User - an OAuth2 instance
 * @param {Object} response - an API Gateway response object
 * @returns {Object} an API Gateway response
 */
async function refreshAccessToken(request, oAuth2User, response) {
  ...
  try {
    ({
      ...
      username,
      expirationTime
    } = await oAuth2User.refreshAccessToken(accessTokenRecord.refreshToken));
  } finally {
 ...
 * @returns {Promise<Object>} a promise of an express response
 */
async function login(request, oAuth2User, response) {
  const code = get(request, 'query.code');
  const state = get(request, 'query.state');

  if (code) {
    return token(request, oAuth2User, response);
  }

  const authorizationUrl = oAuth2User.getAuthorizationUrl(state);
  return buildPermanentRedirectResponse(authorizationUrl, response);
}

function buildGoogleOAuth2UserFromEnv() {
...
function buildEarthdataLoginUserFromEnv() {
  return EarthdataLogin.createFromEnv({
    redirectUri: process.env.TOKEN_REDIRECT_ENDPOINT
  });
}

function buildOAuth2UserFromEnv() {
  return process.env.OAUTH_PROVIDER === 'google'
    ? buildGoogleOAuth2UserFromEnv()
    : buildEarthdataLoginUserFromEnv();
}

/**
 * performs OAuth against an OAuth user
 *
...
async function tokenEndpoint(req, res) {
  const oAuth2User = buildOAuth2UserFromEnv();
  return login(req, oAuth2User, res);
}
...
async function refreshEndpoint(req, res) {
  const oAuth2User = buildOAuth2UserFromEnv();
  return refreshAccessToken(req, oAuth2User, res);
}
