...
const launchpadPublicCertificate = async (launchpadPublicMetadataPath) => {
  ...
  try {
    ...
    if (error.code === 'NoSuchKey' || error.code === 'NoSuchBucket') {
      error.message = `EarthdataPub could not find Launchpad public xml metadata at ${launchpadPublicMetadataPath}`;
    }
    throw error;
  }
 ...
 * @param {string} authorizedGroup - EarthdataPub oauth user group.
