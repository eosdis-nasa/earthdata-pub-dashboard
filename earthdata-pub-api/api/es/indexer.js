/* functions for transforming and indexing EarthdataPub Payloads
 * in ElasticSearch. These functions are specifically designed
 * to transform data for use in earthdata-pub api
 *
...
function parsePayload(payload) {
  let record;
  try {
    // earthdata-pub log message has extra aws messages before the json message,
    ...
    } else { // other logs e.g. earthdata-pub-ecs-task
 ...
 * comply with the earthdata-pub message protocol
 *
 * @param  {Object} esClient - ElasticSearch Connection object
 * @param  {Object} payload  - EarthdataPub Step Function message
 * @param  {string} index    - Elasticsearch index alias (default defined in search.js)
...
/**
 * Indexes the submission type on ElasticSearch
 *
 * @param  {Object} esClient - ElasticSearch Connection object
 * @param  {Object} payload  - EarthdataPub Step Function message
 * @param  {string} index    - Elasticsearch index alias (default defined in search.js)
 * @param  {string} type     - Elasticsearch type (default: submission)
 * @returns {Promise} Elasticsearch response
 */
async function indexSubmission(esClient, payload, index = defaultIndexAlias, type = 'submission') {
  // If the submission exists in 'deletedsubmission', delete it first before inserting the submission
  // into ES.  Ignore 404 error, so the deletion still succeeds if the record doesn't exist.
  const delSubmissionParams = {
    index,
 ...
 * @param  {string} params.index    - Elasticsearch index (default: earthdata-pub)
