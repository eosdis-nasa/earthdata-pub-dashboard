...
const getEarthdataPubMessageFromExecutionEvent = async (executionEvent) => {
  let earthdatapubMessage;

  if (executionEvent.detail.status === 'RUNNING') {
    earthdatapubMessage = JSON.parse(executionEvent.detail.input);
  } else if (executionEvent.detail.status === 'SUCCEEDED') {
    earthdatapubMessage = JSON.parse(executionEvent.detail.output);
  } else {
    const inputMessage = JSON.parse(get(executionEvent, 'detail.input', '{}'));
    earthdatapubMessage = await getFailedExecutionMessage(inputMessage);
  }

  const fullEarthdataPubMessage = await pullStepFunctionEvent(earthdatapubMessage);

  const workflowStatus = executionStatusToWorkflowStatus(executionEvent.detail.status);
  set(fullEarthdataPubMessage, 'meta.status', workflowStatus);

  set(fullEarthdataPubMessage, 'earthdatapub_meta.workflow_stop_time', executionEvent.detail.stopDate);

  return fullEarthdataPubMessage;
};

module.exports = {
  getFailedExecutionMessage,
  getEarthdataPubMessageFromExecutionEvent
};
