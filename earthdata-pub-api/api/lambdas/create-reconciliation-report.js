...
/**
 * Compare the collection holdings in CMR with EarthdataPub
 *
 * @returns {Promise<Object>} an object with the okCollections, onlyInEarthdataPub and
 * onlyInCmr
 */
async function reconciliationReportForCollections() {
  ...
  const okCollections = [];
  let collectionsOnlyInEarthdataPub = [];
  ...
  while (nextDbCollectionId && nextCmrCollectionId) {
    if (nextDbCollectionId < nextCmrCollectionId) {
      ...
      collectionsOnlyInEarthdataPub.push(nextDbCollectionId);
  ...
  collectionsOnlyInEarthdataPub = collectionsOnlyInEarthdataPub.concat(dbCollectionIds);
  ...
  return {
    okCollections,
    onlyInEarthdataPub: collectionsOnlyInEarthdataPub,
    onlyInCmr: collectionsOnlyInCmr
  };
}

/**
 * Compare the file holdings in CMR with EarthdataPub for a given granule
 *
 ...
 * @returns {Promise<Object>} an object with the okCount, onlyInEarthdataPub, onlyInCmr
 */
async function reconciliationReportForGranuleFiles(granuleInDb, granuleInCmr, bucketsConfig) {
  let okCount = 0;
  const onlyInEarthdataPub = [];
  const onlyInCmr = [];
  ...
  granuleInCmr.RelatedUrls.forEach((relatedUrl) => {
    ...
    if (cmrGetDataTypes.includes(relatedUrl.Type)
      ...
      // filename in both earthdata-pub and CMR
  ...
  return { okCount, onlyInEarthdataPub, onlyInCmr };
}
// export for testing
exports.reconciliationReportForGranuleFiles = reconciliationReportForGranuleFiles;

/**
 * Compare the granule holdings in CMR with EarthdataPub for a given collection
 *
...
async function reconciliationReportForGranules(collectionId, bucketsConfig) {
  ...
  while (nextDbItem && nextCmrItem) {
    ...
    if (nextDbGranuleId < nextCmrGranuleId) {
      // Found an item that is only in database and not in cmr
      granulesReport.onlyInEarthdataPub.push({
      ...
      filesReport.onlyInEarthdataPub = filesReport.onlyInEarthdataPub.concat(fileReport.onlyInEarthdataPub);
...
/**
 * Compare the holdings in CMR with EarthdataPub' internal data store, report any discrepancies
 *
 * @param {Object} bucketsConfig - bucket configuration object
 * @returns {Promise<Object>} a reconciliation report
 */
async function reconciliationReportForEarthdataPubCMR(bucketsConfig) {
  const collectionReport = await reconciliationReportForCollections();
  const collectionsInEarthdataPubCmr = {
    okCount: collectionReport.okCollections.length,
    onlyInEarthdataPub: collectionReport.onlyInEarthdataPub,
    onlyInCmr: collectionReport.onlyInCmr
  };

  // create granule and granule file report for collections in both EarthdataPub and CMR
  ...
  const granulesInEarthdataPubCmr = {};
  const filesInEarthdataPubCmr = {};

  granulesInEarthdataPubCmr.okCount = granuleAndFilesReports
    .reduce((accumulator, currentValue) => accumulator + currentValue.granulesReport.okCount, 0);
  granulesInEarthdataPubCmr.onlyInEarthdataPub = granuleAndFilesReports.reduce(
    (accumulator, currentValue) => accumulator.concat(currentValue.granulesReport.onlyInEarthdataPub), []
  );
  granulesInEarthdataPubCmr.onlyInCmr = granuleAndFilesReports.reduce(
    (accumulator, currentValue) => accumulator.concat(currentValue.granulesReport.onlyInCmr), []
  );

  filesInEarthdataPubCmr.okCount = granuleAndFilesReports
    .reduce((accumulator, currentValue) => accumulator + currentValue.filesReport.okCount, 0);
  filesInEarthdataPubCmr.onlyInEarthdataPub = granuleAndFilesReports.reduce(
    (accumulator, currentValue) => accumulator.concat(currentValue.filesReport.onlyInEarthdataPub), []
  );
  filesInEarthdataPubCmr.onlyInCmr = granuleAndFilesReports.reduce(
    (accumulator, currentValue) => accumulator.concat(currentValue.filesReport.onlyInCmr), []
  );

  return { collectionsInEarthdataPubCmr, granulesInEarthdataPubCmr, filesInEarthdataPubCmr };
}
...
async function createReconciliationReport(params) {
  ...
  const filesInEarthdataPub = {
    okCount: 0,
    onlyInS3: [],
    onlyInDynamoDb: []
  };

  const reportFormatEarthdataPubCmr = {
    okCount: 0,
    onlyInEarthdataPub: [],
    onlyInCmr: []
  };
  ...
  let report = {
    ...
    error: null,
    filesInEarthdataPub,
    collectionsInEarthdataPubCmr: cloneDeep(reportFormatEarthdataPubCmr),
    granulesInEarthdataPubCmr: cloneDeep(reportFormatEarthdataPubCmr),
    filesInEarthdataPubCmr: cloneDeep(reportFormatEarthdataPubCmr)
  };
  ...
  bucketReports.forEach((bucketReport) => {
    report.filesInEarthdataPub.okCount += bucketReport.okCount;
    report.filesInEarthdataPub.onlyInS3 = report.filesInEarthdataPub.onlyInS3.concat(bucketReport.onlyInS3);
    report.filesInEarthdataPub.onlyInDynamoDb = report.filesInEarthdataPub
      .onlyInDynamoDb.concat(bucketReport.onlyInDynamoDb);
  });

  // compare the CUMULUS holdings with the holdings in CMR
  const earthdata-pubCmrReport = await reconciliationReportForEarthdataPubCMR(bucketsConfig);
  report = Object.assign(report, earthdata-pubCmrReport);
