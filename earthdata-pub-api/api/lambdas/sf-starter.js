...
function dispatch(message) {
  ...
  input.earthdatapub_meta.workflow_start_time = Date.now();

  if (!input.earthdatapub_meta.execution_name) {
    input.earthdatapub_meta.execution_name = uuidv4();
  }

  return sfn().startExecution({
    stateMachineArn: input.earthdatapub_meta.state_machine,
    input: JSON.stringify(input),
    name: input.earthdatapub_meta.execution_name
  }).promise();
}
