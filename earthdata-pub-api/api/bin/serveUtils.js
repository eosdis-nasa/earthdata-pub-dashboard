...
async function addSubmissions(submissions) {
  const submissionModel = new models.Submission();
  const es = await getESClientAndIndex();
  return Promise.all(
    submissions.map((c) =>
      submissionModel
        .create(c)
        .then((submission) => indexer.indexSubmission(es.client, submission, es.index)))
  );
}
