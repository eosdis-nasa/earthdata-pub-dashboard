...
async function createDBRecords (stackName, user) {
  ...
  const submission = testUtils.fakeSubmissionFactory({ submissionId: `${stackName}-submission` });
  await serveUtils.addSubmissions([submission]);
...
/**
 * Prepare and run the EarthdataPub API Express app.
 *
...
/**
 * Prepare and run the EarthdataPub distribution API Express app.
 *
 * @param {string} stackName - The name of local stack. Used to prefix stack resources.
