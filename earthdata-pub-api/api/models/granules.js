...
const { getMessageGranules } = require('earthdata-pub-api/message/Granules');
...
const {
  ...
  moveGranuleFiles
} = require('earthdata-pub-api/ingest/granule');
...
class Granule extends Manager {
  ...
  async reingest(granule) {
    ...
    // eathdatapub_context cannot be earthdata-pub_context so do not change
  ...
  /**
   * Parse a EarthdataPub message and build granule records for the embedded granules.
   *
   * @param {Object} earthdatapubMessage - A EarthdataPub message
   * @returns {Promise<Array<Object>>} - An array of granule records
   */
  static async _getGranuleRecordsFromEarthdataPubMessage(earthdatapubMessage) {
    const granules = getMessageGranules(earthdatapubMessage);
    if (!granules) {
      log.info(`No granules to process in the payload: ${JSON.stringify(earthdatapubMessage.payload)}`);
      return [];
    }
  ...
  /**
   * Generate and store granule records from a EarthdataPub message.
   *
   * @param {Object} earthdatapubMessage - EarthdataPub workflow message
   * @returns {Promise}
   */
  async storeGranulesFromEarthdataPubMessage(earthdatapubMessage) {
    const granuleRecords = await this.constructor
      ._getGranuleRecordsFromEarthdataPubMessage(earthdatapubMessage);
