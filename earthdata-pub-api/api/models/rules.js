...
const CloudwatchEvents = require('earthdata-pub-api/aws-client/CloudwatchEvents');
const { invoke } = require('earthdata-pub-api/aws-client/Lambda');
...
const workflows = require('earthdata-pub-api/common/workflows');
...
class Rule extends Manager {
  ...
  async addRule(item, payload) {
    ...
    const r = await CloudwatchEvents.putEvent(
      ...
      item.rule.value,
      item.state,
      'Rule created by earthdata-pub-api'
    );
  ...
  static async buildPayload(item) {
    ...
    // rarthdatapub_meta cannot be earthdata-pub_meta, so do not change
    return {
      template,
      definition,
      ...
      meta: get(item, 'meta', {}),
      earthdatapub_meta: get(item, 'earthdatapub_meta', {}),
