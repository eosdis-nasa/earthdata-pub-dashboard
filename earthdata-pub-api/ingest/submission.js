...
const S3 = require('earthdata-pub-api/aws-client/S3');
const log = require('earthdata-pub-api/common/log');

/**
* Copy submission file from one s3 bucket & keypath to another
*
...
**/
function copySubmissionFile(source, target, options = {}) {
...
/**
* Move submission file from one s3 bucket & keypath to another
*
* @param {Object} source - source
* @param {string} source.Bucket - source
* @param {string} source.Key - source
...
**/
async function moveSubmissionFile(source, target, options) {
  await copySubmissionFile(source, target, options);
...
**/
async function moveSubmissionFileWithVersioning(source, target, sourceChecksumObject, copyOptions) {
  ...
  // else rename the existing file, and both files are part of the submission.
  if (targetFileSum === sourceFileSum) {
    ...
    await exports.moveSubmissionFile(
...
async function handleDuplicateFile({
  ...
  if (duplicateHandling === 'error') {
    ...
    return moveSubmissionFileWithVersioning(
      source,
      target,
    ...
    if (syncFileFunction) {
      ...
      await moveSubmissionFile(source, target, copyOptions);
    }
 ...
 * @param {Array<Object>} sourceFiles - submission file objects
 * @param {Array<Object>} destinations - array of objects defining the destination of submission files
...
/**
 * Moves submission files from one S3 location to another.
 *
 ...
 */
async function moveSubmissionFiles(sourceFiles, destinations) {
  ...
  const moveFileRequests = moveFileParams.map((moveFileParam) => {
    ...
    if (target) {
      log.debug('moveSubmissionFiles', source, target);
      return moveSubmissionFile(source, target).then(() => {
        processedFiles.push({
          bucket: target.Bucket,
          key: target.Key,
...
async function renameS3FileWithTimestamp(bucket, key) {
  ...
  return exports.moveSubmissionFile(
...
module.exports.copySubmissionFile = copySubmissionFile;
module.exports.unversionFilename = unversionFilename;
module.exports.moveSubmissionFile = moveSubmissionFile;
module.exports.moveSubmissionFiles = moveSubmissionFiles;
