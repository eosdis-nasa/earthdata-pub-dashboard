  ...
  describe('When logged in', () => {
    ...
    it('displays a link to an individual report', () => {
      ...
      cy.contains('h3', 'Files only in DynamoDB (35)')
        ...
        .find('.table .tbody')
        .as('dynamoTable');
      cy.get('@dynamoTable').find('.tr').should('have.length', 35);
      cy.get('@dynamoTable')
        .children('.tr')
        .first()
        .within(() => {
      ...
      cy.contains('h3', 'Files only in S3 (216)')
        ...
        .find('.table .tbody')
        .as('s3Table');
      cy.get('@s3Table').find('.tr').should('have.length', 216);
      cy.get('@s3Table')
        .children('.tr')
        .first()
        .within(() => {
      ...
      cy.get('earthdata-pub-apiCollectionsTable').find('.tr').should('have.length', 13);
      cy.get('earthdata-pub-apiCollectionsTable')
        .within(() => {
      ...
      cy.get('@cmrCollectionsTable').find('.tr').should('have.length', 25);
      ...
      cy.get('earthdata-pub-apiGranulesTable').find('.tr').should('have.length', 7);
      cy.get('earthdata-pub-apiGranulesTable')
        .within(() => {
      ...
      cy.contains('h3', 'Granules only in CMR (365)')
        ...
        .find('.table .tbody')
        .as('cmrGranulesTable');
      cy.get('@cmrGranulesTable').find('.tr').should('have.length', 365);
      ...
      cy.get('earthdata-pub-apiGranulesFilesTable').find('.tr').should('have.length', 1);
      cy.get('earthdata-pub-apiGranulesFilesTable')
        .children('.tr')
        .first()
        .within(() => {
      ...
      cy.get('@cmrGranulesFilesTable').find('.tr').should('have.length', 1);
