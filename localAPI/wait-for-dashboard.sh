...
until curl --connect-timeout 5 -sS http://localhost:5001/version 2> /dev/null | grep 'api_version' > /dev/null; do
  >&2 echo "Earthdatapub localAPI is unavailable - sleeping"
  sleep 2
done

>&2 echo "LocalAPI is up - checking Localstack"

until curl --connect-timeout 5 -sS curl localhost:3000 2> /dev/null | grep "Earthdatapub Dashboard" > /dev/null; do
