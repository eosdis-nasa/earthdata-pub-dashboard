...
export const CustomUpload = ({requestId, groupId, customComponent, customRequestId}) => {
  const [loaded, setLoaded] = useState(false);
  const [hiddenFileInput, setHiddenFileInput] = useState(React.createRef(null));
  ...
  const handleClick = (e) => {
    e.preventDefault();
    resetInputWithTimeout(undefined, 0);
    handleUpload();
    hiddenFileInput?.current?.click();
  };

  const keyLookup = ({e, fileName}) => {
    e.preventDefault();
    if (keys[fileName]) {
      ...
      if (requestId) {
        ...
        dispatch(listFileDownloadsByKey(keys[fileName], requestId))
        .then(() => {
          download.downloadFile(this.state.keys[fileName], `${apiRoot}data/upload/downloadUrl`, loadToken().token).then((resp) => {
            if (resp?.data?.error || resp?.error || resp?.data?.[0]?.error) console.log(`An error has occurred: ${error}.`);
          })
        })
    }
  }};

  const resetInputWithTimeout = (msg, timeout) => {
    setTimeout(() => {
      if (msg) setStatusMsg(msg);
      if (hiddenFileInput === null || hiddenFileInput.current === null) setHiddenFileInput(React.createRef(null));
    }, timeout);

  };

  const handleCloseUploadSummaryModal = () => {
    setShowUploadSummaryModal(false);
  ...
  const handleFileDrop = (e) => {
    ...
    let localFiles = e.dataTransfer.files;
    if (localFiles.length) {
      setUploadFiles(uploadFiles.concat(Array.from(localFiles)));
    }
  };
  ...
  const handleChange = async(e) => {
    e.preventDefault();
    setUploadFiles(uploadFiles.concat(Array.from(e.target.files)));
  };
  ...
  const tableColumns = [
    {
      Header: 'Filename',
      accessor: row => <><a id={row.file_name} name={row.file_name} aria-label={`Download ${row.file_name}`} onClick={(e) => keyLookup(e, row.file_name)}> {row.file_name}</a></>,
      id: 'file_name'
    },
    {
      Header: 'Size',
      ...
      <div className='page__component'>
        ...
        <div className='indented__details' style={{ paddingTop: '1rem' }}>
          <div className='form__textarea'>
            ...
            {showProgressBar && progressValue > 0 && 
              <div style={progressBarStyle}>
                <div style={progressBarFillStyle}>
                  <span style={numberDisplayStyle}>{uploadFailed ? <span>{'Upload Failed'}<span className="info-icon" data-tooltip={error}></span></span>: `${progressValue}%`}</span>
                </div>
              </div>}
            <div>
            {
              ...
              <div>
                <label style={{
                  ...
                  <input type="radio" id="documentation_category" onChange={() => setCategoryType("documentation")} checked={categoryType === 'documentation'}/>
                ...
                >
                  <input type="radio" id="sample_category" onChange={() => setCategoryType("sample")} checked={categoryType === 'sample'}/>
            ...
            <button onClick={(e) => handleClick(e)} className={`upload-button upload-button-category mt-2 button button__animation--md button__arrow button__arrow--md button__animation button__arrow--white ${(categoryType || groupId) && !showProgressBar? 'button--submit' : 'button--secondary button--disabled'}`}>{uploadStatusMsg === ''? 'Upload': 'Uploading...'}</button>
            {showProgressBar && <span className="d-flex align-items-center">
              <FontAwesomeIcon
          ...
          {saved && requestId !== undefined && groupId === undefined
            ?
            <>
              <br />
            ...
            : null}
          {/* {!loaded && groupId === undefined ? <Loading /> : null} */}
          <span>{saved ? saved : null}</span>
        </div>
